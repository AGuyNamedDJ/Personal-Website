...
const bucket = process.env.NEXT_PUBLIC_S3_BUCKET_ALBUM_APPLE_DAY_23;
...
async function listAlbumObjects() {
  ...
  for (const k of imageKeys) {
    const optMatch = k.match(/^ADO-(\d+)\.(jpe?g)$/i);
    const fullMatch = k.match(/^AD-(\d+)\.(jpe?g)$/i);
    if (optMatch) {
      const id = optMatch[1];
      if (!map[id]) map[id] = {};
      map[id].opt = k;
...
export default async function AppleDayAlbum() {
  ...
  return (
    <main className="relative mx-auto max-w-6xl px-4 py-20 bg-black min-h-screen">
      <h1
        className="relative z-10 whitespace-normal break-words leading-none text-[6rem] md:text-[8rem] font-bold text-center"
